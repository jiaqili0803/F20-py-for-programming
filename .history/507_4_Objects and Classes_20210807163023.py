...
class Dog:
    ...
    #large_dogs，small_dogs: 是class attributes，belong to the entire class
    #是variables that are not attached to self，不属于instance的一部分
    #class attributes对不同instance是不变的一样的，instance也可以访问class attributes
    
    #instance attributes对不同instance有不同value；比如不同dog有different values for name and breed
    
    #eg：all 3 dogs created here will see the same values for large_dogs and small_dogs, even as they see different values for name and breed. 
