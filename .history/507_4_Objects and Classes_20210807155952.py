...
class Dog: #大写，Longer names use CamelCase
  ...
  def __init__(self, nm, br): #constructor，call class时候必用到；self必须是第一个parameter，self refers to the current object；所以only pass two parameters（nm, br）
    self.name = nm
    self.breed = br
# END CLASS DEFINITION

d1 = Dog('Fido', 'German Shepherd')  #instantiate实例化两个objects；两个参数带入（nm, br）
