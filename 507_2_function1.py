...
def func5(var5): #parameter这里叫什么都无所谓，在func5外毫无意义；重名也不影响，因为作用域不同
...
###赋新值的时候，关联变量的变化
#不管是不是mutable，直接赋新值相当于new variable，旧variable相关的东西不变，但是用了method比如append（），就都变了
#str-immutable
s = "hello"
s2 = s 
...
print(s2) # prints "hello" 不变s2还等于原来的s
...
a = [4, 5, 6] #new a
print(a) # prints "[4, 5, 6]"，变了
print(b) # prints "[1, 2, 3]"，不变 b还等于原来的a

a = [1, 2, 3]
b = a
print(a) # prints "[1, 2, 3]"
print(b) # prints "[1, 2, 3]"
a.append(4)  #用了append（），ab都直接一起变了！！！
b.append(5)
print(a) # prints "[1, 2, 3, 4, 5]"
print(b) # prints "[1, 2, 3, 4, 5]"
a = [4, 5, 6]  #new a
print(a) # prints "[4, 5, 6]"，变了
print(b) # prints "[1, 2, 3, 4, 5]"，不跟着new a变化


### Pass a mutable argument to a function
#和之前的区别是 这里的inner没有完全赋值一个新的g_list，所以inner和outer的glist带进带出function说的都是一回事，在完全一个东西上做修改，所以受function影响就变了
#lists、dicts are pointers rather than concrete values, will be modified by functions.
def change_it(the_list):
    the_list.append('d')
g_list = ['a', 'b', 'c']
change_it(g_list)
print(g_list) # prints "['a', 'b', 'c', 'd']",变了
...
print(global_dict['g']) #prints grapefruit 变了
print(global_dict['n']) #prints nectarine 变了
print(global_dict['p']) #prints plum   变了
...
print('4', reminder, groceries[-1]) ##！ 这里变成4 please pick up eggs，reminder没变，groceries变了！！！！！
