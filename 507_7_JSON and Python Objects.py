...
- A brief introduction to unit testing
        一个.py文件是一个module
        要创建multi-module program，选一个作为main.py然后拆开原py就行
        在mian.py里面: 
            import自己定义的其他module; 
            加module名在所引用的funct/class..前面：比如 starwars_character.Character

        当一个module运行时，Python会给其名称为“__main__”并分配给全局变量 __name__
        如果不想让非main的mudule打印乱七八糟的东西：在不想打印的前面写if __name__=="__main__":
'''  
